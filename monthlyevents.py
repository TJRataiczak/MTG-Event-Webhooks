...
for store in c.fetchall():
    #Start webhook
    webhook = DiscordWebhook(url=store[0], username=f"{store[1]} Events", avatar_url=store[2], content="@everyone")
    embed = DiscordEmbed(title=f"{today.strftime('%B')} Events")
    c.execute(f"SELECT eventdate, eventid, eventdescription FROM events WHERE storeid='{store[1]}' AND eventdate LIKE '{today.strftime('%Y')}-{today.strftime('%m')}%'")
    events = c.fetchall()
    #Add all events for the month to embed
    if events != []:
        for event in events:
